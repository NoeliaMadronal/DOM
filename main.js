...
titulo.setAttribute("title","Tienda de Cerveza artesanal");
...
Bienvenid@ a nuestra tienda de cervezas online`;
  ...
  while (edad == "si" || "Si" || "SI") {
      ...
      class Productos {
          constructor(nombre, tamaño, precio, cuotas, info) {
              ...
              this.info = info;
              this.ver = `<ul>
        <li><strong>Cerveza artesanal ${this.nombre} </strong></li>
        <li>Precio: $${this.precio}</li>
        <li>Cuotas: ${this.cuotas}</li>
        <li>Características: ${this.info}</li>
        </ul>
        `;
          }
  ...
  let suscripcion = documento.getElementoById("enviar");
  suscripcion.addEventListener("click", respuestaBtn)
  funtion respuestaBtn(){
